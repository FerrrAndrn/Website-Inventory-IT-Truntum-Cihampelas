...
export default function FbTable({
  initialData,
  tableName = 'housekeeping_equipment'
}: {
  initialData: any[]
  tableName?: string
}) {
  ...
  const downloadQR = () => {
    ...
    ctx!.fillText('HOUSEKEEPING ASSET', newCanvas.width / 2, 35)
              ...
              >
                ...
                <td className="action-cell">
                  <button
                    ...
                    onClick={() =>
                      router.push(`/hk/detail?code=${item.asset_code}`)
                    }
                  >
                    <FiSearch />
                  </button>
          ...
          >
            ...
            >
              ...
              <h3 className="qr-title">
                QR HK ASSETS {qrItem.asset_code}
              </h3>

              <QRCodeCanvas
                id="qr-canvas"
                value={`${process.env.NEXT_PUBLIC_SITE_URL}/hk/detail?code=${qrItem.asset_code}`}
                size={250}
                level="H"
                includeMargin
              />
